...
function [bending_moment_dist] = Find_bending (n_span_points, fuel_weight_dist,lift_dist, wing_length, engine_pos,landing_gear_pos, engine_weight, landing_gear_weight, nu)

bending_moment_dist = zeros(1, n_span_points);

for span_pos = 1:100
    iter_range = span_pos:n_span_points;
    force_distribution = sum((lift_dist(iter_range)-fuel_weight_dist(iter_range)) .* nu(iter_range)) * wing_length;
